...
Dans un scénario de divergence instantanée (Figure \@ref(fig:demographic-models)), sous l'hypothèse que les fréquences alléliques sont distribuées selon une loi normale ou binomiale, $T_{LK}$ suit une loi du $\chi^2$ à $N-1$ degrés de libertés [@lewontin1973distribution]. En effet, en utilisant la définition \@ref(def:fst) de la $F_{ST}$ et en remarquant que :
...
La décomposition de la $F_{ST}$, en une somme d'effets locus-spécifique et population-spécifique, permet d'identifier les locus à forte $F_{ST}$ qui présentent un effet qui n'est pas partagé par les autres locus.
...
Par ailleurs, les méthodes de scan génomique baséss sur des méthodes bayésiennes telles que Bayescan ou la première version de PCAdapt [@beaumont2004identifying; @duforet2014genome] sont connues pour être computationnellement lourdes, et peuvent nécessiter plusieurs jours de calcul même dans le cas de jeux de données comportant seulement quelques milliers de SNPs. 
...
(ref:correlogram-cap) À gauche une matrice d'apparentement génétique interpopulationnelle calculée à partir de la simulation d'un modèle en îles à trois populations. À droite une matrice d'apparentement génétique interindividuelle.
...
En génétique des populations, l'ACP est devenu un outil de visualisation extrêmement utilisé. Cela s'explique notamment par sa capacité à rendre compte de la structure de populations à l'aide d'un faible nombre d'axes principaux (\@ref(fig:hapmap)), appelés aussi *axes de variation génétique* [@price2006principal]. 

(ref:hapmap-cap) ACP réalisée sur la phase 3 du jeu de données HapMap à l'aide de la librarie R pcadapt [@gibbs2003international]. Les deux premières composantes principales distinguent trois groupes génétiques correspondant aux populations africaines, asiatiques et européennes.
...
Une autre particularité de l'ACP réside dans la possibilité d'inférer les *coefficients de métissage* ou *coefficients d'ascendance* à partir des composantes principales [@mcvean2009genealogical; @ma2012principal]. Un coefficient de métissage quantifie pour un individu donné la proportion du génôme provenant d'un groupe génétique spécifique (appelé aussi population source ou population ancestrale). L'un des premiers articles à établir un lien entre l'ACP et les coefficients de métissage global fut sur l'interprétation généalogique de l'ACP de @mcvean2009genealogical (Figure \@ref(fig:mcvean)). Nous reviendrons sur cette notion dans le chapitre consacré à l'introgression.

(ref:mcvean-cap) Coefficients de métissage et ACP [@mcvean2009genealogical]. **A.** Chaque ligne de la figure correspond à un chromosome. Chacune de ces lignes représente la projection des individus issus des populations CEU, ASW et YRI sur la première composante principale. **B.** Chaque ligne de la figure correspond à un chromosome. Chacune de ces lignes représente les projections moyennes, ramenées à l'intervalle $[0, 1]$. 
...
Dans l'article 1 [@duforet2015detecting], nous introduisons la communalité en tant que statistique de test pour la détection de signaux d'adaptation locale. La communalité est une notion empruntée à l'analyse factorielle et s'interprète comme la proportion de variance expliquée par le modèle à facteurs. Le but de notre approche est de proposer une statistique Nous justifierons également d'un point de vue théorique les observations établissant la correspondance entre l'indice de fixation et la communalité. Pour ce faire, nous montrerons dans le cas de deux populations que la communalité peut s'écrire sous une forme analytique semblable à celle de la $F_{ST}$, nous invitant de ce fait à considérer la communalité comme une extension de la $F_{ST}$ à l'échelle individuelle. Cette généralisation est particulièrement intéressante lorsqu'il est difficile de définir des populations de façon claire comme cela peut être le cas en présence d'individus métissés. Cependant, de la même manière que la $F_{ST}$, la communalité n'est pas adaptée au cas de populations structurées. 
...
Le choix de cette statistique est fondé sur l'interprétation usuelle de la communalité en tant que proportion de variance expliquée par une variable. 

#### Résultats principaux {-}

**La $F_{ST}$ peut être interprétée comme une proportion de variance expliquée.**
Pour chacun des scénarios démographiques étudiés (modèle en île et modèle de divergence), la $F_{ST}$ et la communalité présentent en moyenne un degré de corrélation supérieur à $90\%$. Pour établir ce résultat de façon théorique, nous explicitons un modèle à facteurs où la $F_{ST}$ correspondrait à la proportion de variance expliquée, permettant de réécrire la $F_{ST}$ sous une forme analytique analogue à celle de $h^2$.

```{proposition, echo=TRUE}
Soient $N$ le nombre de populations, $n_j$ le nombre d'individus appartenant à la $j$-ème population, $n$ le nombre total d'individus et $\delta_{ji}$ le symbole de Kronecker tel que $\delta_{ji} = 1$ si l'individu $i$ appartient à la $j$-ème population et $0$ sinon. Notant $U_{\delta} = \left(\frac{\delta_{ji}}{n_j}\right)_{1 \leq i \leq n, \; 1\leq j \leq N} \in M_{n, N}(\mathbb{R})$, il existe une matrice $L \in M_{Np}(\mathbb{R})$ telle que :

$$\tilde{G} = U_{\delta}L.$$
```

```{proof, echo=TRUE}
En constatant que $U_{\delta}^TU_{\delta} = \text{Diag}\left(\frac{1}{n_1}, \dots, \frac{1}{n_N}\right)$, nous pouvons définir $L = (U_{\delta}^TU_{\delta})^{-1}U_{\delta}\tilde{G}$, si bien que $\tilde{G} = U_{\delta}L$.
```
...
L'interprétation de la $F_{ST}$ en termes de proportion de variance expliquée fait de la communalité une statistique de test intéressante d'un point de vue conceptuel, permettant de considérer la communalité comme une extension de la $F_{ST}$. Cependant, elle garde de la $F_{ST}$ les défauts liés à l'apparentement génétique interpopulationnel (cf. équation \@ref(eq:LK-chi2)). Afin de remédier à ce problème, nous proposons une statistique de test basée sur la distance robuste de Mahalanobis.

### La distance robuste de Mahalanobis

Ce paragraphe résume de façon détaillée les résultats obtenus dans l'article 2, assortis de justifications théoriques qui ne figurent pas nécessairement dans l'article.

#### Définition {-}

```{definition, mahalanobis, name="Distance de Mahalanobis", echo=TRUE}
Soit $x \in \mathbb{R}^K$, et $X \in M_{pK}(\mathbb{R})$ une matrice représentant un ensemble de $p$ observations $K$-dimensionnelles. La distance de Mahalanobis de $x$ relativement à cet ensemble est donnée par la formule ci-dessous : 

\begin{equation}
    D^2(x) = (x - \bar{x})^T \Sigma^{-1} (x - \bar{x}),
\end{equation}

où $\bar{x}$ (resp. $\Sigma$) désigne la moyenne des observations (resp. la matrice de covariance).

Si $X$ suit une loi normale multidimensionnelle d'espérance $\bar{x}$ et de matrice de covariance $\Sigma$ définie positive, alors $D^2 \sim \chi_p^2$.
```

La distance de Mahalanobis est une statistique classique de détection de valeurs aberrantes pour des données multivariées. La distance de Mahalanobis est utilisée pour déterminer si une observation appartient effectivement à un ensemble d'observations. Dans notre cas, cet ensemble d'observations correspond aux locus neutres. 
Géométriquement 


Nous avons vu dans la partie précédente que la communalité avait tendance à accorder beaucoup d'importance aux premières composantes principales, du fait de la pondération des loadings par les valeurs singulières. 
...
La statistique de test $T_{F-LK}$ et la $F_{ST}$ généralisée proposée par @ochoa2016f sont basées sur une estimation de la matrice d'apparentement génétique $\mathcal{F}$ par une *méthode des moments*. Pour des données gaussiennes multivariées distribuées, cela consiste à estimer les moments d'ordre 1 et 2 que sont la moyenne et la variance. Les estimateurs classiques de moyenne sont connus pour être sensibles à la présence de données aberrantes. Pour cette raison, nous avons choisi d'utiliser un estimateur dit *robuste* de la matrice de covariance. Nous avons comparé les méthodes MCD (Déterminant Mininum de la Covariance) et OGK (Gnanadesikan-Kettenring Orthogonalisé) implémentées dans la librairie R `robust` (Figure \@ref(fig:ogk-mcd)). 
...
df.div <- rbind(df.data, df.out, df.2, df.3)
...
df.2 <- get.ellipse.coord(obj.ogk, "OGK")
df.3 <- get.ellipse.coord(obj.mcd, "MCD")
df.isl <- rbind(df.data, df.out, df.2, df.3)
...
**La distance robuste de Mahalanobis est adaptée à des scénarios démographiques variés.**
Nous montrons que la distance robuste de Mahalanobis donne de meilleurs résultats en termes de puissance statistique, indépendamment du modèle démographique sous-jacent (à l'exception du modèle en îles où les résultats sont équivalents). 

**En comparaison avec les autres méthodes de scan génomique, la distance robuste de Mahalanobis permet de mieux contrôler le taux de fausses découvertes.**
