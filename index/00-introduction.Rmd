...
Un caractère peut survenir de différentes manières :

- le caractère est déjà présent dans la population.

- le caractère apparaît dans la population à la suite d'une mutation spontanée.

- le caractère est hérité d'une autre population par flux de gène.

L'identification des mutations génétiques à l'origine d'épisodes d'adaptation locale est particulièrement cruciale pour la compréhension des processus évolutifs et plus spécifiquement ceux liés à la spéciation^[épisode de divergence d'une espèce ayant conduit à la formation d'au moins une nouvelle espèce]. Bien que l'adaptation locale puisse être étudiée de façon expérimentale^[par le biais de la *transplantation réciproque* [@kawecki2004conceptual]], les données de séquençage permettent d'appréhender le problème de façon plus quantitative.

#### L'hybridation et le flux de gène comme sources d'adaptation

La reproduction sexuée est connue pour jouer un rôle important dans la sélection naturelle, offrant par le biais du brassage allélique la possibilité aux individus d'hériter des "meilleurs allèles" que possèdent leurs parents. Pour enrichir le catalogue d'allèles, nous avons vu un peu plus haut que le flux de gène constituait un processus clé. De la même manière, l'hybridation entre deux espèces peut permettre à l'une des deux espèces d'intégrer de nouveaux allèles [@stevison2008hybridization]. La sélection d'allèles transmis par flux de gène ou par hybridation correspond à ce qu'on appelle l'*introgression adaptative*.
...
Le séquençage nouvelle génération (appelé encore séquençage à haut débit) a connu un essor considérable au cours des dernières décennies. Si bien que les prouesses techniques et les progrès technologiques réalisés dans ce domaine ont permis de réduire d'un facteur 100,000 les coûts de séquençage en l'espace de seulement 15 ans \@ref(fig:wetterstrand). 
...
(ref:microsat) Exemples de microsatellites. Un microsatellite est repérable par la répétition successive de petits motifs chacun composé de 1 à 4 acides aminés. La première séquence comporte 3 répétitions du motif CCG, tandis que la seconde inclut 4 répétitions du motif CA.
...
(ref:snp-cap) Le polymorphisme d'un seul nucléotide correspond au polymorphisme génétique le plus simple, et correspond à l'emplacement d'un nucléotide présentant des variations appréciables à l'échelle d'une population.
...
La production de données génétiques, volumineuses par la quantité d'information qu'elles renferment, laisse présager le meilleur pour les domaines de la médecine clinique et de la biologie évolutive. En génétique des populations, leur acquisition offre de nombreuses perspectives d'étude, notamment concernant la mise en évidence de marqueurs génétiques impliqués dans les processus évolutifs. Les méthodes répondant à cette problématique portent le nom de *scans génomiques*. 

Toutefois, certains outils statistiques ayant été développés ne sont plus adaptés. La raison principale étant que les temps de calculs requis pour l'analyse de ces données massives sont souvent prohibitifs. Une méthode d'analyse exploratoire nécessite parfois d'être 
expérimentée plusieurs fois. D'une part parce que le prétraitement des données peut être est amené à changer au cours de l'étude. Des marqueurs génétiques ou des individus peuvent par exemple être retirés du jeu de données si ceux-ci présentent une proportion trop importante (le degré d'importance étant fixé par un seuil fixé de façon arbitraire) de données manquantes. D'autre part car les méthodes d'analyse proposent généralement un éventail de paramètres dont le choix ne s'impose pas spontanément. La possibilité d'expérimenter une même méthode avec différents critères de qualité et différents paramètres est donc primordiale pour minimiser les biais d'utilisation. Bien évidemment cela ne dépend pas uniquement des algorithmes utilisés et il faut aussi tenir compte des ressources de calcul mises à disposition des utilisateurs. Nous avons néanmoins pris le parti de proposer un outil permettant de réaliser des scans génomiques pour la sélection sur des données pouvant contenir jusqu'à plusieurs millions de marqueurs génétiques génotypés pour plusieurs milliers d'individus, sans que cela ne requiert une utilisation démesurée de mémoire vive.