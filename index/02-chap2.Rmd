...
Dans la partie précédente nous nous sommes intéressés à la détection d'allèles ayant favorisé l'adaptation d'une population à son environnement. Cependant, les scans génomiques pour la sélection ne permettent pas de comprendre l'origine du mécanisme adaptatif sous-jacent (mutation aléatoire ou flux de gène). Savoir si un allèle sélectionné a été hérité ou non d'une autre population par flux de gène peut donc se révéler intéressant dans ce cas. Nous présentons dans ce chapitre différents outils statistiques pour la détection de signaux d'introgression.

## Méthode basée sur l'utilisation de coefficients de métissage locaux.

Nous explicitons ici le principe de détection de signaux d'introgression à partir de la donnée de coefficients de métissage locaux. 

### Coefficients de métissage locaux

Nous avons vu un peu plus tôt qu'il était possible d'estimer pour un individu, la proportion de son génôme provenant d'un ou de plusieurs groupes génétiques. Ces proportions sont connues plus communément sous le nom de *coefficients de métissage* et peuvent servir à mieux comprendre l'histoire démographique des populations métisséees. De nombreux logiciels existent pour l'estimation de ces coefficients : STRUCTURE, ADMIXTURE [@alexander2009fast], LEA [@frichot2015lea], tess3r [@caye2016tess3]. Chaque individu peut de ce fait être vu comme une mosaïque d'allèles qui ont été puisés dans des groupes génétiques distincts. En revanche, les coefficients de métissage ne renseignent pas sur les portions du génôme qui proviennent effectivement de ces groupes génétiques. Chercher à attribuer à chaque allèle la population ancestrale de laquelle il est originaire constitue la problématique d'estimation des *coefficients de métissage locaux*. Avoir à disposition une telle information individuelle et locale permettrait ainsi de quantifier à l'échelle de la population la probabilité qu'un allèle ait été fourni par une population spécifique. Ce type de méthodologie, basée sur l'estimation de coefficients de métissage locaux, a par exemple été employé dans l'étude de peupliers d'Amérique du Nord [@suarez2016]. Encore une fois, plusieurs logiciels ont été proposés dans le but d'estimer ces coefficients locaux : HAPMIX [@price2009sensitive], EILA [@yang2013efficient], LAMP [@thornton2014local], Loter [A COMPLETER] ou encore RFMix [@maples2013rfmix].

(ref:LAI-cap) Exemple de matrice de coefficients de métissage locaux pour des individus issus du métissage de deux populations sources (numérotées 1 et 2). **A**. Chaque ligne de la matrice correspond à un SNP et chaque individu est représenté par une paire de colonnes (de la même couleur). **B**. Matrice de coefficients de métissage locaux individuels. L'exemple illustre que pour chaque nucléotide d'un individu, les méthodes d'estimation de coefficients de métissage locaux attribuent ce nucléotide soit à la population 1 soit à la population 2.
...
plot_grid(p1, p2, p3, p3, labels = c("A", "B", "C", "D"))
```

### Statistique de test

À partir de la matrice de coefficients de métissage locaux individuels, il est possible d'estimer pour un SNP donné la proportion d'allèles provenant de chacun des populations sources considérées. Notant $M$ cette matrice et $n$ le nombre d'individus métissés, cette proportion s'écrit :

\begin{equation}
  P(G_{j,.} \in \text{P}_K) = \frac{1}{n}\text{Card}(\{i \in [|1, n|] \; | \; M_{ji} = K \})
\end{equation}

La recherche de régions d'introgression dans la population hybride se fait ensuite en regardant pour chaque $(P(G_{j,.} \in \text{P}_K))_{1 \leq j \leq p}$, les SNPs présentant des proportions significativement élevées par rapport aux autres.

## Méthodes basées sur la diversité nucléotidique.

La seconde classe de méthodes repose sur la comparaison de séquences de nucléotides, nucléotide par nucléotide. Un chromosome est par exemple divisé en un certain nombre de séquences (qui peuvent être disjointes ou chevauchantes), et les statistiques sont calculées sur chacune de ces séquences. Les séquences de nucléotides sont vues comme des chaînes de caractères et leur dissimilarité est mesurée à l'aide de distances classiques (ou de mesures de comptage) telles que la distance de Hamming.

### Diversité nucléotidique par paire de séquences {-}
...
$d_{XY}$ est appelée la *pairwise nucleotide diversity* et estime le nombre moyen de nucléotides différents entre une séquence venant de la population $X$ et une séquence venant de la population $Y$. $d_{\min}$ estime la distance minimale entre deux séquences tirées de $P_1$ et de $P_2$. 

Pour détecter l'introgression entre deux populations $X$ et $Y$ à partir de distances entre séquences nucléotidiques, la seule donnée de $d_{XY}$ ou de $d_{\min}$ ne suffit pas, une distance n'étant une mesure de proximité que si elle est mise en comparaison avec une autre distance. C'est pour cela que les statistiques d'introgression basées sur ce type de distance requièrent généralement la donnée d'une population de contrôle, généralement notée O. C'est le cas par exemple des statistiques $RND$ et $RND\min$ définies ci-dessous [@rosenzweig2016powerful] :
...
Ces populations sont généralement présentées selon le schéma suivant :

(ref:123O-cap) Arbre allélique.
...
Sous l'hypothèse nulle, c'est-à-dire en l'absence d'introgression, la proportion de séquences communes à $P_3$ et $P_1$ doit être la même que la proportion de séquences communes à $P_3$ et à $P_2$. À l'inverse, un évènement d'introgression entre $P_3$ et $P_1$ devrait s'accompagner de l'augmentation de la fraction de séquences partagées par $P_3$ et $P_1$, relativement à $P_2$. Cette différence de proportions est mesurée par la statistique $D$ de Patterson [@durand2011testing], conçue spécifiquement pour détecter l'introgression. De façon plus imagée, notant $A$ et $B$ respectivement les allèles portés par $O$ et par $P_3$, la statistique $D$ compte le nombre de motifs $ABBA$ (Figure \@ref(fig:abba-baba)) et le nombre de motifs $BABA$ (Figure \@ref(fig:abba-baba)) et teste si les deux quantités sont significativement différentes l'une de l'autre.
...
En termes mathématiques, la statistique $D$ de Patterson, définie à partir des motifs $ABBA$ et $BABA$, est donnée par la relation suivante :

\begin{equation} 
  D = \displaystyle \frac{\sum_i C_{ABBA}(i) - C_{BABA}(i)}{\sum_i C_{ABBA}(i) + C_{BABA}(i)} 
\end{equation}

où $C_{ABBA}(i)$ (resp. $C_{BABA}(i)$) désigne le nombre de motifs $ABBA$ (resp. $BABA$) observés sur le site $i$-ème nucléotide de la séquence.
De nombreuses variantes ont été développées à partir de cette statistique, dont les statistiques $Bd_{f}$ et $f_d$ auxquelles nous nous comparons [@pfeifer2017estimates; martin2014evaluating], mais nous renvoyons le lecteur aux références correspondantes pour des détails supplémentaires.

### Analyse en Composantes Principales locale

Notant $p$ le nombre de marqueurs génétiques, $i$ un entier compris entre $1$ et $p$, et $x_i$ la position génétique (mesurée en Morgans) ou la position physique (mesurée en paires de bases) du $i$-ème marqueur génétique. Nous définissons pour cet entier $i$ la fenêtre $W_i^T$ de taille $T$ et centrée en $i$ :

\begin{equation}
  W_i^T = \{ j \in [|1, p|], |x_i - x_j| \leq T/2 \}
\end{equation}

Soit $\tilde{G} = U \Sigma V^T$ la décomposition en valeurs singulières de la matrice de génotypes normalisée $\tilde{G}$. Partant de l'égalité $U = \tilde{G}V\Sigma^{-1}$, nous définissons des scores locaux de la façon suivante :
...
L'heuristique de cette approche repose sur l'interprétation des loadings $V$. Nous rappelons que chaque composante principale est la combinaison linéaire des SNPs, pondérée par les loadings. En reprenant l'assertion que les composantes principales permettent de discriminer les populations, Les loadings mesurent donc à quel point un SNP est informatif pour la séparation des populations. 
