...
Soit $U\Sigma V^T$ la décomposition en valeurs singulières de $\tilde{G}$. Notons $\mathcal{F}$ la matrice d'apparentement génétique interpopulationnel. En utilisant l'estimateur usuel de la matrice de covariance, nous avons :
...
\begin{equation}
  \begin{split}
    ...
    &\simeq \tilde{G}_{.,j}^T U_{\delta} (U_{\delta}^T U \Sigma ^2 U^T U_{\delta})^{-1} U_{\delta}^T \tilde{G}_{.,j} 
  \end{split}
\end{equation}
...
```{r ndf-figS9, results='asis', fig.cap='(ref:ndf-figS9-cap)'}
