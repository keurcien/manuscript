...
Si nous calculions $U_{W_i^T}$ pour chaque valeur de $i$ de façon naïve, la complexité algorithmique de cette étape serait en $O(nKp^2)$. Pour réduire cette complexité, l'idée est d'exploiter le fait que les fenêtres $W_i^T$ et $W_{i+1}^T$ se chevauchent très probablement, ce qui permet de déduire $U_{W_{i+1}^T}$ à partir de $U_{W_i^T}$ en effectuant moins d'opérations que n'en nécessiterait le calcul individuel de $U_{W_{i+1}^T}$.
...
```{r chr06-100kbp, results='asis', fig.cap='(ref:chr06-100kbp-cap)', out.width="75%"}
...
De la même manière, pour évaluer notre méthode dans un scénario d'introgression, nous reprenons les modèles de simulation proposés par @martin2014evaluating. L'idée est la même que précédemment. Nous définissons 4 populations et nous générons pour chacune d'entre elles des séquences de nucléotides. Une population est désignée receveuse et une seconde population est désignée donneuse. Pour la population receveuse, nous désignons de plus une région dans laquelle les séquences de la population receveuse sont mélangées à des séquences de la population donneuse, simulant de cette manière un évènement d'introgression. Les résultats de cette comparaison sont donnés en figure \@ref(fig:power-f). Nous constatons que dans le cas où taux de mutation est constant le long du génôme, notre méthode obtient des performances similaires aux autres méthodes. En revanche, dans le cas où le taux de mutation est variable, elle semble être la moins robuste de toutes.
...
For the introgression scenario, we compared pcadapt to the $Bd_f$, $D$, $f_d$, and $RND_{\text{min}}$ test statistics [@durand2011testing; @martin2014evaluating; @pfeifer2017estimates; @rosenzweig2016powerful]. The $D$, $Bd_f$ and $f_d$ statistics require genotype data for the three related species as well as from an outgroup species, for which we also simulate genotypes (\@ref(tab:stats_introgression)). Because $RND_{\text{min}}$ tests for introgression between sister species, we provide sequence (haplotype) data from 3 species, which are the introgressed, and donor species as well as an outgroup species (Figure \@ref(fig:scenarios)). When using pcadapt, we provide genotype data from three species, which are introgressed, donor and sister species (Figure \@ref(fig:scenarios)).
