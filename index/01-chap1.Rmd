...
Dans le cas de structure hiérarchique (panel D de la figure \@ref(fig:demographic-models)), les quantités $p_i - \bar{p}$ ne sont plus indépendantes entre elles, ce qui remet en cause l'approximation $\chi^2$. Pour tenir compte de la structure hiérarchique, @bonhomme2010detecting proposent de corriger la statistique $T_{LK}$ pour l'apparentement génétique des populations, modélisé par une matrice $\mathcal{F} = (f_{ij})_{1 \leq i,j \leq N} \in M_N(\mathbb{R})$, où $f_{ij}$ mesure la corrélation entre $p_i$ et $p_j$ et peut être interprétée comme la probabilité qu'un individu de la population $i$ et un individu de la population $j$ aient hérité de cet allèle d'un même ancêtre commun.
